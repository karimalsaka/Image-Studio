      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {features.map(({ icon, title, description }, i) => (
          <FadeIn key={title} delay={0.1 + i * 0.08}>
            <div className="bg-[var(--surface-raised)] rounded-2xl border border-[var(--border)] p-5 hover:shadow-md hover:shadow-black/4 transition-all">
              <div className="w-8 h-8 rounded-lg bg-[var(--surface-inset)] flex items-center justify-center mb-3">
                {icon}
              </div>
              <h3 className="font-semibold text-sm text-[var(--text-primary)] mb-1.5">{title}</h3>
              <p className="text-[var(--text-tertiary)] text-sm leading-relaxed">{description}</p>
            </div>
          </FadeIn>
        ))}
      </div>
